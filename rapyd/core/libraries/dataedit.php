...
class dataedit_library extends dataform_library
{
	...
	protected function delete_button($config = null)
	{
		...
		}elseif($this->status_is('modify') && rpd_url_helper::value('modify' . $this->cid)){
			$delete_url = rpd_url_helper::replace('modify' . $this->cid, 'delete' . $this->cid);
			$action = "javascript:window.location.href='" . $delete_url . "'";
			$this->button('btn_delete', $caption, $action, 'TR');
	...
	public function process()
	{
		...
		switch ($this->status)
		{
			...
			case "delete":
				$this->build_buttons();
				return $this->build_message('Seguro que desea eliminar el registro?');
				break;
			case "unknow_record":
				$this->build_buttons();
				return $this->build_message('Registro no reconocido');
				break;
		}
	}
